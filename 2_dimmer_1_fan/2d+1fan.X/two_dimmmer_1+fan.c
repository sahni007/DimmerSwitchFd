...
#include <xc.h>
#define RELAY1 RF1 // 2nd SW
#define FAN RF0 //1sw sw
#define RELAY2 RE5//DIMMER1
#define RELAY3 RA3//DIMMER2
#define RELAY4 RA2//DIMMER3

#define REGULATOR RE0 // regulator 
#define FAN1 RE2
#define FAN2 RE4
#define FAN3 RE1

#define SW5 RF7//DIMMER1
#define SW4 RF5//DIMMER2
#define SW3 RF3//DIMMER3
#define SW2 RF2//switch for fan
#define SW1 RA5
...
int man=1;
unsigned int child_LOCK[10]="00000000000";
...
/*manual Response started */
	//switch   
			if(child_LOCK[1]==OFF && SW1==OFF && M1==OFF)
			{
                if(man==1)
                {
                st[1]='R';      st[2]='0';      st[3]='0';      st[4]='1';      writeUART(st+1);        FAN=OFF;
                }
				M1=1;                man=1;
	        }
			...
			if(child_LOCK[1]==OFF && SW1==ONN && M1==ONN)
			{
                if(man==1)
                {
                st[1]='R';      st[2]='1';      st[3]='0';      st[4]='1';      writeUART(st+1);		 FAN=ONN;
                }
				M1=0;                man=1;
			}
	 //fan 
 //...............FOR FAN...................................................//      
        if(child_LOCK[3]==OFF && SW2==OFF && M2==OFF)
			...
			if(child_LOCK[5]==OFF && SW3==OFF && M3==OFF)
			{
                if(man==1)
                {
                st[9]='R';     st[10]='0';     st[11]='0';     st[12]='3';      writeUART(st+9);FAN_FLAG1=0;
				RELAY4=1; 
                }
                M3=1;
                man=1;
			...
			{ 
                if(man==1)
                {
                st[9]='R';        st[10]='1';       st[11]='0';       st[12]='3';     writeUART(st+9);FAN_FLAG1=0;
				RELAY4=0;
                }
                M3=0;
                man=1;
	       ...
	       if(child_LOCK[7]==OFF && SW4==OFF && M4==OFF)
			{    
                if(man==1)
                {
                st[13]='R';      st[14]='0';       st[15]='0';       st[16]='4';     writeUART(st+13);FAN_FLAG2=0;
				RELAY3=1;                 
				}
                M4=1;               
                man=1;
			...
			if(child_LOCK[7]==OFF && SW4==ONN && M4==ONN)
			{    
                if(man==1)
                {
                st[13]='R';     st[14]='1';        st[15]='0';       st[16]='4';    writeUART(st+13);    FAN_FLAG2=0;   
				RELAY3=0;
                }
                M4=0;                
                man=1;
       ...
       // sw5
            if(child_LOCK[9] == OFF && SW5==OFF && M5==OFF)
			{    
                if(man==1)
                {
                st[17]='R';
			...
			if(child_LOCK[9] == OFF && SW5==ONN && M5==ONN)
			{     
                if(man==1)
                {
                st[17]='R';
...
void ACTION(char Switch_Num_10s, char Switch_Num_1s, char sw_status, char speed_bitHigh, char speed_bitLow, char parent,char finalFrameStat)
{
    ...
    switch(SwNum)       // char SwNum
    {
        ...
        case 2:
        {
                 M1=switch_status;       //M1-->For FAN
            if(switch_status==0)                                           // SPEED 0
            {
                __delay_ms(500);
 //               TX1REG='F';
//                REGULATOR=ONN;
//                __delay_ms(1);
            ...
            else if( speed_bitHigh=='2')             // speed 1
            {
                __delay_ms(2);
//                TX1REG='1';
                 __delay_ms(500);
...
//                 __delay_ms(1000);
            ...
            else if( speed_bitHigh=='5')         // speed 2
            {
                __delay_ms(1);
   //             TX1REG='2';
//                __delay_ms(1000);
//                REGULATOR=ONN;
            ...
            else if( speed_bitHigh=='7')      // speed 3
            {                
                __delay_ms(1);
     //           TX1REG='3';
                  __delay_ms(500);
...
//                __delay_ms(1000);
            ...
            else if(speed_bitHigh=='9')                         // speed 4
            {   
                __delay_ms(1);
 //               TX1REG='4';
                __delay_ms(500);
...
//     TRISGbits.TRISG4=1;   //ccp5
     ...
     TRISFbits.TRISF2=1;            // switch 4//fan switch
...
//     TRISFbits.TRISF6=1;
     ...
     WPUE=0x00;
     ANSELE=0x00;
    TRISEbits.TRISE2=0;     // regulator           
    TRISEbits.TRISE4=0;     // FAN 1
    TRISEbits.TRISE0=0;     // FAN 2             
    TRISEbits.TRISE1=0;     // FAN 3  
